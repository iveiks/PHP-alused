...
if (!empty($_POST)) {
    ...
    if (empty($uname) || empty($password) || empty($confirm_password)) {
        ...
        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            ...
            $insert_stmt = mysqli_prepare($yhendus, "INSERT INTO clients (username, password_hash) VALUES (?, ?)");
            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "ss", $uname, $hashed_password);
